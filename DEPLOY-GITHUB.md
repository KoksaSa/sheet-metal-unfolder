...
Версия бандла: **v5.6** (подставляется автоматически скриптом `.zscripts/make-github-bundle.sh`).
...
```bash
cd github-release && git init && git add . && git commit -m "v5.6"
```
...
1. В репозитории: **Releases → Create a new release → Choose a tag** → введите новый тег
   `v5.6` → Target: `main`.
2. Название: `Sheet Metal Unfolder v5.6`; в описании можно взять раздел
   «Что нового» из `README.md`.
3. **Attach binaries**: перетащите zip-архив приложения — готовый архив уже собран рядом с
   бандлом: `public/downloads/sheet-metal-unfolder-github-v5.6.zip`
